...
public class WoodenBasinBlock extends BlockWithEntity implements Waterloggable{
    ...
    public void appendTooltip(ItemStack stack, @Nullable BlockView world, List<Text> tooltip, TooltipContext options) {
        if(Screen.hasShiftDown()){
            ...
            tooltip.add(Text.translatable(ModUtil.WOODEN_BASIN_1).formatted(Formatting.GOLD));
            tooltip.add(Text.translatable(ModUtil.WOODEN_BASIN_2).formatted(Formatting.GOLD));
            tooltip.add(Text.translatable(ModUtil.WOODEN_BASIN_3).formatted(Formatting.GOLD));
        } else {
